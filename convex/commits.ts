...
export const getLiveCommits = query({
    ...
    handler: async (ctx, args) => {
        ...
        // Query only last 10 minutes of commits
        // Reactivity: This query will re-run whenever new commits are added within this window
        const query = ctx.db.query("commits")
            .withIndex("by_timestamp", (q) => q.gt("timestamp", tenMinutesAgo));

        const commits = [];
        // Scan limit isn't strictly needed if we trust volume in 10 mins is reasonable,
        // but safe to keep.
